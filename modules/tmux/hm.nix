...
let
  ...
  runtimePath = "${cfg.stateDir}/tmux/hot-stylix-current.conf";
  configPath = "${config.xdg.configHome}/tmux/themes/hot-stylix-current.conf";
  templatePath = "${hotStylixInputs.stylix.inputs."tinted-tmux".outPath}/templates/base16.mustache";
in
{
  ...
  config = lib.mkMerge [
    {
      hotStylix.supportedTargets.tmux = {
        enable = config.programs.hot-stylix.targets.tmux.enable;
        inherit runtimePath;
    ...
    (lib.mkIf config.programs.hot-stylix.targets.tmux.enable {
      ...
      xdg.configFile."tmux/themes/hot-stylix-current.conf".source =
